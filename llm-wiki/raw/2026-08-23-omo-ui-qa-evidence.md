...
6. Issue 16 later verified the monthly-average KPI against the same elapsed prior-year period. The displayed title and prior-period label identify the same January-to-current-month range; the comparison includes explicit `▲`/`▼` direction, percentage, and formatted prior KRW amount using the shared KPI typography and positive/negative color rules. When the prior total is not positive, the entire comparison row is hidden. The 375px layout wraps without horizontal overflow.
